...
import { reactSelectStyles, portalSelectProps } from "../utils/selectStyles";
import "./product-manager-features.css";
// Excel Import/Export is shown only in development builds.
// Production builds hide the buttons completely.
// .env / .env.development  -> VITE_APP_ENV="development"
...
function normalizeProducts(products) {
  ...
  return products.map((p) => ({
    ...
    discountAmount: Number(p.discountAmount ?? p.discount_amount) || 0,
    discountMode:
      p.discountMode === "value" || p.discount_mode === "value"
        ? "value"
        : Number(p.discountPercent ?? p.discount_percent ?? 0) === 0 &&
            Number(p.discountAmount ?? p.discount_amount ?? 0) > 0
          ? "value"
          : "percent",
    discountValue:
      p.discountMode === "value" || p.discount_mode === "value"
        ? Number(p.discountValue ?? p.discountAmount ?? p.discount_amount ?? 0)
        : Number(
            p.discountValue ?? p.discountPercent ?? p.discount_percent ?? 0,
          ),
    status:
      p.status === PRODUCT_STATUS.NO_STOCK
...
function buildImportPayload(product) {
  return {
    ...
    maxOrderQty:
      product.maxOrderQty === null || product.maxOrderQty === ""
        ? null
...
export default function ProductManager({
  ...
  const soldProductSet = useMemo(
    () =>
      new Set(
        (Array.isArray(propSoldProductNames) ? propSoldProductNames : []).map(
          normalizeText,
        ),
      ),
    [propSoldProductNames],
  ...
  const showLocalAlert = useCallback(
    (title, message, variant = "info", confirmText = "OK") => {
      if (showAlert) {
        showAlert(title, message, variant, confirmText);
      } else {
        setAlert({
  ...
  const handleAdd = useCallback(
    async (payload) => {
      ...
      try {
        ...
        setProducts((prev) => [...prev, normalizeProducts([newProduct])[0]]);
        setCategories((prev) => [
          ...new Set([...prev, newProduct.category].filter(Boolean)),
        ]);
        showLocalAlert("Success", "Product added successfully", "success");
      } catch (err) {
        console.error("Add error:", err);
  ...
  const handleUpdate = useCallback(
    async (payload) => {
      ...
      try {
        ...
        setProducts((prev) =>
          prev.map((p) =>
            p.rowid === rowid ? normalizeProducts([updatedProduct])[0] : p,
          ),
        );
        setCategories((prev) => [
          ...new Set(prev.concat(updatedProduct.category).filter(Boolean)),
        ]);
        showLocalAlert("Success", "Product updated successfully", "success");
      } catch (err) {
        console.error("Update error:", err);
      ...
      setAlert({
        ...
        onConfirm: async () => {
          ...
          try {
            ...
            const remaining = products.filter((p) => p.rowid !== rowid);
            setCategories([
              ...new Set(remaining.map((p) => p.category).filter(Boolean)),
            ]);
            showLocalAlert(
              "Success",
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((p) => {
      ...
      const matchesCategory =
        !internalCategory || p.category === internalCategory;
      const matchesStatus = !internalStatus || p.status === internalStatus;
      const matchesSoldProducts =
        soldProductSet.size === 0 || soldProductSet.has(normalizeText(p.name));
      const matchesBrand =
        !brandFilter || normalizeBrand(p.brand) === brandFilter;
      return (
        matchesSearch &&
        matchesCategory &&
        matchesStatus &&
  ...
  const handleExportExcel = useCallback(() => {
    ...
    try {
      ...
      showLocalAlert(
        "Export complete",
        `${products.length} product(s) exported to Excel.`,
        "success",
      );
    } catch (err) {
      console.error("Excel export error:", err);
      showLocalAlert(
        "Export failed",
        err.message || "Unable to export products to Excel.",
        "danger",
      );
    }
  }, [products, showLocalAlert]);
    ...
    async (importedProducts) => {
      ...
      try {
        ...
        showLocalAlert(
          ...
          err.response?.data?.message ||
            err.message ||
            "Unable to import products. The existing catalog was not changed.",
          "danger",
        );
      } finally {
        setIsSaving(false);
  ...
  const handleImportExcel = useCallback(
    async (file) => {
      ...
      try {
        ...
        showLocalAlert(
          "Invalid Excel file",
          err.message || "Please use the exported Products Excel format.",
          "danger",
        );
      }
    },
      ...
      <div className="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        <div className="d-flex align-items-center gap-2">
          ...
          {(internalCategory ||
            internalStatus ||
            brandFilter ||
            soldProductSet.size > 0) && (
            <Badge bg="warning" className="ms-2">
              <i className="bi bi-funnel me-1"></i>{" "}
              {soldProductSet.size > 0 ? "Sold products" : "Filtered"}
            </Badge>
          )}
        </div>
        <div className="d-flex gap-2 flex-wrap">
          ...
          <Button
            ...
            disabled={isSaving}
          >
            <i className="bi bi-plus-lg me-1" aria-hidden="true"></i> Add
            Product
          </Button>
        </div>
      </div>
                        ...
                        <div className="d-flex flex-row">
                          ...
                          >
                            ...
                            {((product.discountMode === "value" &&
                              ...
                              >
                                {product.discountMode === "value"
                                  ? `₹${formatINR(product.discountAmount)} OFF`
                          ...
                          <Card.Body className="d-flex flex-column flex-grow-1 p-2">
                            ...
                            <div className="d-flex gap-2 mt-3">
                              ...
                              <Button
                                ...
                                className="flex-fill stock-toggle"
                                onClick={() =>
                                  handleToggleStatus(product.rowid)
                                }
                                title="Toggle stock"
                              >
                                <i
