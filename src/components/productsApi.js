...
const toBoolean = (value, fallback = false) => {
  ...
  if (['true', '1', 'yes', 'y', 'active', PRODUCT_STATUS.IN_STOCK].includes(text)) return true;
  if (['false', '0', 'no', 'n', 'inactive', PRODUCT_STATUS.NO_STOCK].includes(text)) return false;
  return fallback;
};
...
export function normalizeProductForExcel(product = {}) {
  ...
  return {
    ...
    status: product.status ?? PRODUCT_STATUS.IN_STOCK,
    ...
    reader.onload = (event) => {
      try {
        ...
        const rows = rawRows.map((raw, index) => {
          ...
          const status = String(mapped.status || PRODUCT_STATUS.IN_STOCK).trim().toLowerCase();
          if (![PRODUCT_STATUS.IN_STOCK, PRODUCT_STATUS.NO_STOCK].includes(status)) {
            throw new Error(`Row ${rowNumber}: status must be in_stock or no_stock.`);
          }

          const brand = normalizeBrand(mapped.brand);
