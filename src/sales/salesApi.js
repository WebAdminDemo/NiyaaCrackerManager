...
const normalizeOrder = (order = {}) => ({
  ...order,
  customer: {
    name: order.partyName || order.customerName || "Walk-in customer",
    phone: order.partyNumber || order.customerPhone || "—",
    address:
      order.partyAddress ||
  ...
  partyName: order.partyName || order.customerName || "Walk-in customer",
  partyNumber: order.partyNumber || order.customerPhone || "—",
  partyAddress:
    order.partyAddress ||
    order.customerAddress ||
    order.customer?.address ||
  ...
  partyCountry: order.partyCountry || "India",
  ...
  brandMode: order.brandMode === "standard" ? "standard" : "multiBrand",
  customerName: order.partyName || order.customerName || "Walk-in customer",
  customerPhone: order.partyNumber || order.customerPhone || "—",
  customerAddress:
    order.partyAddress ||
    order.customerAddress ||
    order.customer?.address ||
  ...
  items: (order.items || []).map((item) => ({
    ...
    originalPrice: Number(
      item.originalPrice ?? item.original_price ?? item.price ?? 0,
    ),
    ...
    brands: Array.isArray(item.brands)
      ...
      : item.brand
        ? [item.brand]
        : ["Standard"],
    catalogMode: item.catalogMode === "multibrand" ? "multibrand" : "standard",
  })),
});
...
export const salesApi = {
  ...
  async updateOrder(orderId, details, items) {
    const response = await api.patch(
